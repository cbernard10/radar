...
from radarpkg.processing.dataset_to_burg import dataset_to_burg_jit
from ..visualisation.visualisation import colorize, convert_to_jpeg, red


def burg_reshape(burg_data):

    """ reshape the burg coefficients for umap input """
...
def burg_to_umap(burg_data, n_neighbors, min_dist, metric='euclidean', low_memory=True, verbose=0, save=0):
    ...
    """ performs the umap algorithm on burg coefficients """
...
def burg_to_umap_3d(burg_data, n_neighbors, min_dist, metric='euclidean', low_memory=False, verbose=0):
    try:
        formatted_data, colors, power_colors = burg_reshape(burg_data)
    except IndexError as e:
        print('Invalid data, skipping dataset' + repr(e))
        ...
        try:
            if verbose:
                print('mapping...')
            mapper = umap.UMAP(n_neighbors=n_neighbors, n_components=3, min_dist=min_dist, metric=metric,
                               low_memory=low_memory, verbose=verbose).fit(
                formatted_data)
            if verbose:
                print('mapping done')
...
def umap_to_images(umap_data, dataset, filename, s=0.01, subFolder=''):
...
def dataset_to_burg_to_umap_save(dataset, subsampling=1, order=4, n_neighbors=10, min_dist=0, metric='euclidean',
    ...
    try:
        burg_data = dataset_to_burg_jit(dataset, subsampling, order, save=save)
        umap_data, colors, power_colors = burg_to_umap(burg_data, n_neighbors, min_dist, metric)
    except IndexError as e:
        print('Invalid data, skipping dataset' + repr(e))
    except TypeError as e:
        print('Invalid data, skipping dataset' + repr(e))
