...
#
#     # print(P, mus)
#     return np.concatenate((P, mus)), a[1:]
...
#     for i in f(range(X)):
#         for j in range(Y):
#             coeffs[i, j], _ = burg_reg_jit(CData[i, j], order, gamma)
#
#     return coeffs, _
...
# def dataset_to_burg(dataset, order, gamma, subsampling=1, progress=False, save=False, suffix=''):
#     try:
...
#
#         coeffs, _ = burg_reg_3d_jit(cdata, order, gamma, progress)
#
#         if save:
#             for i in range(1, order):
#                 fig, ax = map_and_scatter(coeffs[:, ::-1, i], ranges, times, 1)
#                 if not (os.path.isdir('../csir/' + dataset + '/burg')):
#                     os.mkdir('../csir/' + dataset + '/burg/')
#                 im_path = '../csir/' + dataset + '/burg/' + 'c' + str(i) + suffix + '.png'
...
#             plt.savefig(
#                 '../csir/' + dataset + '/burg/' + 'c0' + '.png', bbox_inches='tight', dpi=200)
#             plt.close(f)
#
#         return coeffs
...
def dataset_to_burg_jit(dataset, order, gamma, subsampling=1, progress=False, save=False, suffix=''):
    try:
        ...
        #     im_path = '../csir/' + dataset + '/burg/' + 'c0' + suffix + '.png'
        #     if progress:
        #         print(im_path)
        #     plt.savefig(im_path, bbox_inches='tight', dpi=200)
        #     plt.close(fig)
...
# def apply_burg_datasets(datasets, order, start=None):
#     if datasets == 'all':
...
#             dataset_to_burg(dataset, order, save=True)
#         else:
